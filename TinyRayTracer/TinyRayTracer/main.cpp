...
//  TinyRayTracer
//
//  Created by liuxing on 2023/6/24.
//
...
#pragma opm parallel for
    for (int pix = 0; pix < width * height; pix++) { // actual rendering loop every pixel
        float dir_x = (pix % width + 0.5) - width / 2.;
        float dir_y = -(pix / width + 0.5) + height / 2.; // this flipx the image at the same time
